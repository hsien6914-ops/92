...
# שרת פשוט כדי ש-Render לא יתנתק (Dummy Server)
...
def create_pdf(products, filepath):
    ...
    for i, p in enumerate(products, 1):
        name_rev = p['name'][::-1] # היפוך לעברית פשוטה
        pdf.cell(0, 10, txt=f"{i}. {name_rev} : {p['barcode']}", ln=1, align='R')
    pdf.output(filepath)
            ...
            if res.get("result"):
                for update in res["result"]:
                    ...
                    if "message" in update:
                        ...
                        print(f"Got message from {chat_id}, sending PDF...")
